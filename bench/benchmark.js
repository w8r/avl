...
const rvalues = new Array(N)
  .fill(0)
  .map((n, i) => Math.floor(Math.random() * N));
...
new Benchmark.Suite(`Random read (x${N})`, options)
  .add("Bintrees", () => {
    for (let i = N - 1; i; i--) {
      prefilledRB.find(rvalues[i]);
    }
  })
  .add("Functional red black tree", () => {
    for (let i = N - 1; i; i--) {
      prefilledFRB.get(rvalues[i]);
    }
  })
  .add("Google Closure library AVL", () => {
    for (let i = 0; i < N; i++) {
...
new Benchmark.Suite(`Remove (x${N})`, options)
  .add("Bintrees", () => {
    for (let i = 0; i < N; i++) {
      prefilledRB.remove(rvalues[i]);
    }
  })
  .add("Functional red black tree", () => {
    for (let i = 0; i < N; i++) {
      prefilledFRB = prefilledFRB.remove(rvalues[i]);
    }
  })
  .add("Google Closure library AVL", () => {
    for (let i = 0; i < N; i++) {
      prefilledGCAVL.remove(rvalues[i]);
    }
  })
  .add("AVL (current)", () => {
    for (let i = N - 1; i; i--) {
      prefilledAVL.remove(values[i]);
    }
  })
  .run();
